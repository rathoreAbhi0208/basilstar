...
# Using Zerodha Kite (polling) instead of TrueData websocket
...
class Scanner:
    ...
    def start(self):
        print("Connecting to Zerodha Kite (polling mode)...")
        time.sleep(2)

        kite_client = kite

        symbols = NIFTY200_SYMBOLS
        print(f"Subscribing to {len(symbols)} symbols (polling)...")

        # Initialize stores for symbols
        ...
        # Polling loop: fetch quote for each symbol and trigger evaluation on change
        while True:
            try:
                for symbol in symbols:
                    try:
                        ...
                        data = kite_client.quote(key)
                        info = data.get(key)
                        if not info:
                            continue

                        ltp = float(info.get('last_price') or info.get('last_price') == 0 and 0 or info.get('last_price', 0))

                        # Only act if LTP actually changed
                        if last_ltp.get(symbol) == ltp:
                            continue
                ...
                time.sleep(0.5)  # avoid rate limits; adjust as needed
